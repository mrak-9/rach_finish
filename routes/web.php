...
// Регистрация на конференции (ограничение от множественных регистраций)
...
// Загрузка файлов (строгие ограничения)
Route::get('/publications/{publication}/download/{file}', [PublicationController::class, 'download'])
...
// Платежные операции (защита от злоупотреблений)
Route::post('/membership/payment', [MembershipController::class, 'payment'])
    ->middleware('throttle:web-forms')
    ->name('membership.payment');

Route::post('/membership/qr', [MembershipController::class, 'generateQR'])
    ->middleware('throttle:web-forms')
    ->name('membership.qr');
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('cabinet')->name('cabinet.')->group(function () {
        ...
        // Обновление профиля (ограничение от частых изменений)
        Route::put('/profile', [UserCabinetController::class, 'updateProfile'])
            ->middleware('throttle:web-forms')
            ->name('profile.update');
            
        // Смена пароля (строгие ограничения)
        Route::put('/password', [UserCabinetController::class, 'updatePassword'])
        ...
        // Загрузка тезисов (ограничение загрузки файлов)
        Route::post('/thesis/{participation}', [UserCabinetController::class, 'uploadThesis'])
            ->middleware('throttle:file-upload')
            ->name('thesis.upload');
    });
});
